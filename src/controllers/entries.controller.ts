...
export const entriesController = async (req: Request, res: Response) => {
    const start = Date.now();
    const filter = req.query.filter as string | undefined;

    try {
        const filterType = filter && isValidFilter(filter) ? filter : undefined;
        const result = await entryService.getEntries(filterType);

        usageLogService.createLog({
        ...
        res.status(502).json({ error: "Failed to fetch entries", detail: message });
    }
};